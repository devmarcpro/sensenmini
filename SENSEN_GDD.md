# 森森 Sensen — Game Design Document

*Document de production — v1.0 (2026-08-09). Intègre l'ensemble des amendements de la session de conception. Le combat directionnel façon Mount & Blade et le mode tactique ont été abandonnés au profit d'un **combat temps réel de placement et de rythme** ; la chimie élémentaire a été supprimée.*

> **Ordre de construction du projet** (décision 2026-08-09, après l'abandon du prototype Voxen) :
> **0.** prototype de combat isolé (document séparé) — **1.** génération du monde seule — **2.** ~30 matériaux justifiés un par un — **3.** combat rapatrié dans le monde — **4.** boucle de donjon complète — **5.** craft compositionnel — **6.** Wu Xing et chaînes.
> PNJ, villages, royaumes et économie viennent **après** un jeu jouable et amusant sans eux. Une seule langue au départ (clés `tr()` dès le début, CJK plus tard).
> **Règle absolue :** une brique à la fois, chacune avec un critère de sortie formulé AVANT de commencer ; rien ne démarre tant que la précédente n'est pas passée.

---

## 1. Concept général

**Pitch :** Un RPG voxel en monde infini généré procéduralement et totalement continu, où une carte du monde façon roguelite (case par case) sert de couche stratégique et de voyage rapide au-dessus d'un monde voxel 3D façon Minecraft — avec construction à résolution variable, un **combat temps réel de placement et de rythme** structuré par les cinq éléments du **Wu Xing**, une progression par l'usage à la Elona/Elin, et un endgame de construction de royaume.

**L'identité du jeu tient en une phrase :** un jeu de **décisions**, pas de dextérité — ce qui gagne un combat, c'est de savoir quel élément suit lequel, quelle arme dégainer et quand détoner sa chaîne, pas la précision de la souris.
...
- Bloc de base : 16 pixels.
- Subdivision possible : 16 → 8 → 4 → 2 → 1 pixel (résolution fractale, façon octree).
...
- **Blocs subdivisés :** le fantôme montre la **sous-cellule exacte** visée dans le bloc parent (indispensable pour placer un bloc de 4px ou 1px avec précision).
- **Structures et modèles sculptés :** le fantôme affiche le modèle entier ; **rotation via les touches directionnelles** avant validation. Placement **aligné sur la grille 16px, centré, par défaut** ; en maintenant **Shift**, le placement bascule sur la **grille fine** (jusqu'à 1px) pour un positionnement précis (ex : coller un meuble exactement contre un mur).
...
### 4.2.1 Craft compositionnel (remplace les recettes plates)
...
**Wu Xing composite (5.2) :** l'alignement élémentaire de l'objet dérive de ses composants — une arme manche-bois/tête-métal est alignée **Bois ET Métal** (chaque alignement au prorata du poids du composant). Tout-métal = fort contre Bois mais vulnérable au Feu ; mixte = polyvalent sans bonus franc. Le choix des matériaux de composants devient un choix d'alignement.

**Niveaux de recette — les doublons approfondissent :** chaque recette a **5 niveaux** ; apprendre une recette déjà connue la fait monter (coût croissant : N doublons pour passer au niveau N, soit 10 au total). Aucun parchemin n'est un loot mort. L'enseignement par un artisan à haute relation (7.2) devient le moyen **volontaire** de cibler une recette précise. *Axe de bonus à trancher au playtest* (efficacité matière / vitesse et lots / stabilité du jet) — **contrainte non négociable : le niveau de recette ne multiplie jamais la qualité**, sinon farmer des parchemins court-circuite la progression de compétence.
...
*(La chimie élémentaire — Extracteur/Synthétiseur, 58 éléments — a été **supprimée** le 2026-08-09 : trop lourde en contenu pour un système purement endgame, et redondante avec le Wu Xing sur « de quoi est fait un matériau ». Une seule couche élémentaire subsiste, dérivée de la catégorie (5.2). Le rôle d'endgame d'artisanat est repris par les **recettes industrielles** — voir 4.2.2.)*

### 4.2.2 Palier industriel (endgame d'artisanat)

Le joueur **trouve et achète des recettes industrielles** (ruines profondes, marchands des capitales, rangs hauts de guilde) : aciers alliés, béton, verre trempé, brique réfractaire, caoutchouc. Aucun système nouveau — ce sont des entrées de plus en B.13, avec des `unlock_sources` exigeantes et des stations améliorées (Forge → Haut fourneau, Enclume → Laminoir) consommant du **combustible** (coke/charbon), ce qui redonne un usage aux minerais communs de fin de partie.
...
### 5.0 Le temps : temps réel unique

**Le jeu se joue entièrement en temps réel.** Le mode tactique (tour par tour basculable) a été **supprimé** le 2026-08-09 : une attaque visée ne se découpe pas naturellement en tours, et le système laissait des pans entiers du jeu orphelins. L'identité roguelike de Sensen vit dans ses **systèmes** (progression par l'usage, potentiel, donjons, jets hors combat, permadeath-lite A.10), pas dans un tour par tour.

- **Architecture à ticks conservée** : toute la logique de simulation (IA, statuts, croissance, économie, faim) est pilotée par des ticks (10/s), jamais par le delta de frame — contrainte d'architecture fondamentale (Annexe D). Ce qui disparaît, c'est la *bascule* comme mécanique de jeu, pas l'horloge.
- **Les durées d'action sont en secondes** : vitesse de déplacement en blocs/s, wind-up et durée de balayage par arme, temps d'incantation par module. Les anciens « coûts en ticks » d'E.1 sont convertis (÷10) comme premières valeurs d'équilibrage.
- **Pause** : en solo, ouvrir l'inventaire ou une station met le jeu en pause simple. En multijoueur, jamais de pause.
- **Vote** : la mécanique de vote ne subsiste que pour le **saut de nuit** (E.21) — majorité simple, fenêtre 15 s, re-vote après 30 s.

### 5.1 Combat

**Combat de placement et de rythme** (décision 2026-08-09 — remplace à la fois les jets de toucher et le combat directionnel façon Mount & Blade, dont l'exigence d'animation est hors de portée d'un développeur solo en voxel) :

- **La géométrie décide du toucher.** Chaque arme a une portée et un arc ; ce qu'elle atteint, elle le touche. Pas de jet de toucher, pas de chance de rater.
- **Un seul bouton d'attaque, le contexte décide du coup** — le joueur choisit sa frappe **en se déplaçant**, jamais en tirant la souris : à l'arrêt = coup standard · en avançant = estoc (perforant, allonge +1) · en reculant = coup d'arrêt rapide · en sprintant = charge (contondant, recul). Quatre coups distincts, zéro input à apprendre, lisibles à l'écran par le mouvement.
- **Attaque lourde au clic maintenu** : c'est le **seul wind-up du jeu**, télégraphié par une lueur montante et un son — un effet de particules, pas une animation. Même effet pour le joueur et pour les créatures : une seule chose à produire.
- **Zones dérivées de la géométrie**, jamais visées : la hauteur du point d'impact décide (au-dessus des épaules = tête ×2.5, torse ×1.0, jambes ×0.8 + ralentissement). C'est ce qui rend le **terrain voxel mécaniquement pertinent** — pentes, marches et hauteurs deviennent des outils.
- **Allonge et distance** : chaque arme a une portée *et* un minimum efficace (−40 % de dégâts en dessous). Le pas en avant et le pas en arrière sont les vrais inputs.
- **Garde active** (clic droit maintenu) : réduit les dégâts frontaux de 80 %, coûte de l'endurance **à l'impact** (pas de drain passif). **Parade parfaite** dans une fenêtre de 0.25 s avant l'impact : dégâts annulés, endurance rendue, adversaire chancelant. Un seul timing à apprendre.
- **Rupture de garde** : endurance à zéro en gardant → chancellement.
- **Endurance longue et lente** (A.6.1) : ~12 coups légers depuis le plein, ~15 s pour refaire le plein. Un combat se gère sur la durée, pas coup par coup.
- **Les jets de dés survivent hors combat** : le **jet de compétence universel** (1d20 + compétence/2 + stat/4 vs difficulté) reste la résolution de la lecture (A.7), du dressage, de la négociation, de la discrétion (E.26), de la capture. L'échec critique (1 naturel) n'existe plus en combat, il demeure sur ces jets.
- **Les dés de dégâts restent** : chaque arme a ses dés (E.3), c'est leur *résolution* qui a changé, pas leur expression.
...
- **Visée temps réel des modules** : les modules-projectiles sont des **projectiles physiques visés à la souris** (esprit Noita assumé) ; les zones au sol s'indiquent au curseur avec télégraphe visible ; les effets sur soi ou en aura s'activent instantanément.
- **Progression infinie par l'usage (façon Elin) :**
  ...
  - **L'XP de combat vaut les dégâts appliqués** (5.3) — trois pistes simultanées.
- **Statuts de contrôle** : étourdissement et enracinement sont des pertes de contrôle **à durée affichée**. Aucun contrôle dur ne dépasse 2 s sur le joueur, et ne peut se réappliquer dans les 5 s suivant sa fin (anti-stunlock), joueur comme créatures.
- **Compagnons** : les ordres passent par une **roue d'ordres temps réel** (maintien d'une touche → suivre / tenir / attaquer ma cible / repli / posture), avec ralenti à 20 % en solo pendant que la roue est ouverte, jamais en multijoueur.
...
- **Côté défense, les multiplicateurs sont compressés** (×1.20 / ×0.85 / ×0.95) : un mauvais matchup défensif est un désagrément, jamais un mur — l'armure est un choix d'identité permanent, pas une variable à optimiser avant chaque donjon (6.2).
...
- **JAUGE DE CHAÎNE** — le cœur du combat (détail A.4.6) : une barre de **5 segments** (extensible jusqu'à 10), toujours visible. Tout acte qui touche pose un segment de son élément ; le 5ᵉ **résout** et reçoit tout le bonus accumulé. Chaque transition rapporte selon sa nature : même élément **+0.10**, changement hors ordre **+0.20**, changement suivant l'engendrement **+0.35**. Comme les places sont comptées, **répéter un élément interdit mathématiquement la rotation complète** — la contrainte est structurelle, aucune interdiction à écrire.
...
- **L'élément d'une ARME est celui de sa TÊTE** (les poids de slots A.4.7 garantissent qu'elle domine). Une arme **mono-élément** se fabrique en engageant tous ses composants dans la même famille — le coût est matériel et réel (un manche métallique est dense et conducteur, une tête en bois est faible).
- **Les ARMES FANTOMATIQUES sont pures par nature** : invoquées, sans composants, donc toujours `{élément: 1.0}` — multiplicateurs pleins, segment net. En contrepartie : dégâts moindres (~×0.7), entretien en mana, ni sertissables ni enchantables, et progression sur le **niveau d'élément et la Volonté** au lieu des matériaux. C'est la seule source fiable de pureté, et l'accès au cycle des invocateurs.
- **Alignement des cibles** : dérivé du champ `elements` (B.5) ou des tags pour une créature ; du **vecteur composite des composants** pour un personnage équipé (A.4.7). L'armure de plates face à un mage de Feu est un vrai handicap.
- **MODIFICATEURS D'AFFINITÉ** (affixes, anneaux, buffs, gemmes taillées en affinité) : quatre opérations sur le vecteur — **ajout**, **amplification**, **purification** (pousse vers le mono-élément — la voie qui rend une arme mixte tranchante), **transmutation** (« remplace X par Y »). Résolution ordonnée puis **normalisation à 1** : un ajout dilue toujours les autres parts, il n'est jamais gratuit. Un ajout suffisant fait **basculer la dominante**, donc le segment de chaîne, les matchups et la piste d'XP : ces effets déplacent l'identité, ils n'ajoutent pas des chiffres.
- **Cinq accès au cycle, chacun payé dans une monnaie différente** : le **râtelier** du guerrier (Force + or — la capacité de port finance l'arsenal, et changer d'arme en pleine chaîne est une mécanique voulue) · le module **Communion des cinq** (mana d'entretien + rythme : l'élément de l'arme tourne tout seul) · l'**enchantement déclencheur** (slots + découverte : l'arme avance sa propre chaîne à chaque coup touché) · les **anneaux de transmutation** (le cycle complet lui-même : fermer un élément interdit la rotation mais rend la concentration naturelle) · les **armes fantomatiques** (mana + puissance brute).
...
- **Alchimie et cuisine (7.7)** : chaque ingrédient porte une affinité élémentaire (dérivée de sa catégorie/nature, surchargée en données) ; un **plat équilibré sur les cinq éléments** gagne un bonus de nutrition et de potentiel (+20 %) — l'assiette harmonieuse du daoïsme, mécanisée.
...
Versée **en entier à trois pistes simultanément** : l'**élément** du coup (le même dominant qui a rempli le segment de chaîne), la **compétence d'arme** (ou le domaine de magie pour un module), et la **maîtrise de type de dégâts** (tranchant / perforant / contondant). Le multiplicateur de **potentiel** (6.4) s'applique ensuite, par piste.

**Côté défense, symétrie exacte** : l'armure gagne ce qu'elle **épargne** — `dégâts évités` versés à la **compétence de construction** de la pièce touchée (Matelassé, Cuir, Mailles, Écailles, Plaque). Une zone nue ne rapporte rien, et la compétence de construction augmente en retour la réduction de la pièce.
...
Parité totale avec les PNJ et compagnons qui progressent (E.17).
...
- **Date de naissance choisie librement** (sélecteur avec aperçu des effets), qui détermine le **signe du cycle sexagésimal chinois** : un **élément** (cycle de 5 ans) et un **animal** (cycle de 12 ans) — 60 combinaisons. Les PNJ ayant déjà un âge (12.2), leur signe en **dérive gratuitement**.
  - **Élément de naissance** : +10 de potentiel de base dans les domaines liés (Bois → Foudre/Vie, Agriculture · Feu → domaine Feu, Cuisine, Forge · Terre → Terre, Minage, Construction · Métal → Métal, Forge, façonnage · Eau → Eau/Glace, Alchimie, Navigation).
  - **Animal** : +10 de potentiel dans 2 compétences thématiques (Rat → Discrétion/Négociation · Bœuf → Encaissement/Agriculture · Tigre → Deux Mains/Athlétisme · Lapin → Esquive/Herboristerie · Dragon → Contrôle du Mana/Leadership · Serpent → Alchimie/Perception · Cheval → Athlétisme/Dressage · Chèvre → Tissage/Taille de pierre · Singe → Lecture/Dextérité fine · Coq → Armes à distance/Perception · Chien → Dressage/Encaissement · Cochon → Cuisine/Négociation).
  ...
  - **Tout passe par le potentiel** (6.4), jamais par un bonus dur : la naissance donne une pente, pas un plafond.

### 6.2 Équipement : 14 slots, armure par zone

**Principe directeur** — l'armure est **permanente**, l'arme est **situationnelle**. Le système de rotation situationnelle existe déjà et s'appelle le râtelier ; l'armure définit l'identité durable du personnage. Le jeu ne doit jamais récompenser le transport de cinq panoplies.

**La grille (14 slots) :**
- **Armure (5), mappée sur les zones de coup** : Casque (tête, la zone ×2.5) · Cuirasse (torse) · Brassards-gants (bras et mains) · Jambières (jambes) · Bottes (pieds, + effets de déplacement).
...
- **Munitions (1)** : carquois (munitions compositionnelles : pointe + hampe).
...
`dégâts_finaux = (dégâts_bruts × zone_mult − armure_zone)`, minimum 1, avec `armure_zone = dureté_composite / 4 × qualité × (1 + niveau_construction / 100)`. **Zone nue = 0** : pas de casque, la tête prend le ×2.5 en plein. La garde reste **la** défense active ; l'armure est le filet.

**Les « types » d'armure n'existent pas comme étiquettes — ils émergent des CONSTRUCTIONS**, qui sont des recettes de composant (B.13) avec leur profil contre les types de dégâts :
...
**Emplacements par morphologie** : quadrupède = tête, torse, selle, amulette, 2 accessoires · volant = tête, torse, amulette, 2 accessoires · amorphe = amulette, 2 accessoires (`equip_slots`, B.5).
...
- **Recrutement : la relation individuelle est le critère** (B.5) ; les réputations race/royaume agissent en **modificateur de vitesse** du gain de relation (×0.5 à ×1.5 selon le palier), jamais en seuil direct. Les **compatibilités astrologiques** (6.1) s'y ajoutent comme second modificateur de vitesse.

**L'INFORMATION EST LA RÉCOMPENSE PRINCIPALE DE LA RELATION** (avant les prix). Deux axes se dévoilent par paliers :

**Ce qu'on sait du PNJ** — sa fiche s'ouvre progressivement : < 0 apparence seule · 0-19 nom, métier, royaume · 20-49 âge, signe, humeur, niveau approximatif · 50-74 liens familiaux, compétences chiffrées, équipement · 75-89 préférences de cadeau **explicites** (au lieu d'être devinées), spécialités, opinions sur les autres PNJ · 90-100 historique complet et relations exactes. Un PNJ hostile ne cache rien : il ne se confie simplement pas.

**Ce qu'il sait du monde** — filtré par **métier** autant que par palier (sinon tous les PNJ deviennent le même distributeur ; le filtrage par métier existe déjà en E.23) :
...
- 50-74 : informations **à valeur marchande** — position d'un filon majeur, tanière d'une bête rare, **quelle ville possède quel hall de guilde ou boutique** (l'unicité par ville, E.27, rend cette information précieuse), position d'un donjon non découvert
- 75-89 : **savoirs transmissibles** — un artisan **enseigne une recette de composant exotique** (troisième source du craft compositionnel, avec le loot et l'achat), un érudit indique un domaine, un alchimiste une potion
...
**Voie de rédemption** (la partie négative doit avoir une sortie) : dérive naturelle de **+1 par semaine** vers 0 · réparation légale (amende payée, E.26) · **quêtes de rachat** sous −50 qui remontent la relation collective à −20 d'un coup. **Garde-fou : au moins un canal de dialogue reste toujours ouvert** dans chaque royaume (un marginal, un ermite, un contrebandier) — un joueur ne peut jamais être socialement muré.
...
- **Harmonie Wu Xing (5.2/A.4.6)** : chaque ingrédient porte une affinité élémentaire ; un plat couvrant les cinq éléments gagne ×1.2 en nutrition et potentiel — l'équilibre daoïste de l'assiette, mécanisé.
...
- **Blocs de construction** (taille normale, 16px) : texture de bruit en **16×16 pixels** (1 pixel de bruit par voxel, à la résolution de subdivision la plus fine).
...
| **Mode tactique supprimé** | jeu 100 % temps réel ; votes réduits au sommeil ; roue d'ordres pour les compagnons |
| **Combat directionnel (M&B) abandonné** | remplacé par le **placement et le rythme** : contexte de déplacement, un seul wind-up, zones dérivées de la géométrie — faisable sans animateur |
...
- **Lore** : noms propres, textes d'ambiance, mythologie — à écrire au fil du contenu.
- **Audio/musique** : hors périmètre de ce document.
- **Saisons** (E.28 : l'architecture les accueille, gros impact agricole — après playtest de la boucle agricole).
- **Créatures fantastiques** en zones à haute corruption (F.3 : prévu sans changement de système).

### À trancher au playtest (implémentable sans)

- Axe unique des **niveaux de recette** (efficacité matière / vitesse et lots / stabilité).
- **Esquive active** dédiée ou mouvement pur.
- Répartitions élémentaires exactes d'**Arcane, Espace, Corruption**.
- Fourchettes des **gemmes** et plafond +15 par compétence ; taille des pools d'**affixes**.
- Ce qui compense réellement une **arme mixte** face à une arme pure (au-delà de l'amortissement des matchups et de la purification par gemmes).

### Contenu à produire

- 5 modules du domaine **Métal** (F.2) ; affinités élémentaires des ingrédients de cuisine.
- Recettes d'obtention par composant × famille (B.13) et leurs sources exotiques.
- Pools de noms des 10 cultures (C.9).
- Surcharges `wuxing` des matériaux dont la catégorie ment.

### Trous connus du combat (à traiter avant ou pendant le prototype)

- **Multi-ennemis** : règles d'engagement, gestion du dos, un balayage touche-t-il plusieurs cibles, chaque cible touchée remplit-elle un segment de jauge ?
- **Vocabulaire d'attaque des créatures** : patterns télégraphiés et variés (le prototype en teste un seul type).
- **Fuite et désengagement** : rayon de perte d'intérêt, rupture de ligne de vue, semer en Discrétion — sans quoi chaque rencontre est un duel à mort.
- **Chaîne côté ennemis** : les créatures ont-elles une jauge (adversaires à interrompre) ou le système est-il purement joueur ?
- **Boucliers** : un bouclier occupe une main dans un système qui récompense la rotation d'armes — quelle compensation ?
- **Projectiles** : vitesse, gravité, portée max, interaction avec les blocs destructibles, friendly fire en coop.
...
### A.4.6 Wu Xing : vecteurs, domination et jauge de chaîne (5.2)
...
JAUGE DE CHAÎNE — 5 SEGMENTS DE BASE, EXTENSIBLE JUSQU'À 10 :
  ...
  DÉCROISSANCE : un segment se vide toutes les 3 s sans acte qui
    touche (le dernier posé en premier). C'est une CADENCE, pas un
    stock que l'on traverse le donjon avec.
  ...
  RISQUE PROPORTIONNEL : une chaîne longue se charge plus lentement
    et s'expose davantage à l'interruption. Étendre sa jauge est un
    pari, pas un bonus gratuit.

MODIFICATEURS D'AFFINITÉ (affixes, anneaux, buffs, gemmes taillées
en affinité) — quatre opérations sur le vecteur :
  AJOUT         « +X % de [élément] »
...
COÛTS : coup standard 8 · estoc 8 · coup d'arrêt 6 · charge 15
        attaque lourde 18 · garde À L'IMPACT 12 + dégâts/4
        sprint 5/s · dégainage d'arme (râtelier) 5
RÉGÉNÉRATION : 6/s après 1.5 s sans dépense ; 2/s si la garde est
  levée. Pas de drain passif de garde.
À ZÉRO : attaque lourde et garde impossibles, coups légers à 60 %
  de dégâts, état « essoufflé » visible.
RUPTURE DE GARDE : endurance à 0 en gardant → chancellement 1.2 s.
PARADE PARFAITE : garde levée dans les 0.25 s avant l'impact →
  dégâts annulés, +10 endurance, adversaire chancelant 0.6 s.
Ordre de grandeur : ~12 coups légers depuis le plein, ~15 s pour
  refaire le plein.
```

### A.4.7 Craft compositionnel : stats et qualité (4.2.1)
...
### A.12 Loot : affixes, gemmes et rareté (4.2.1/6.2)
...
### B.1 Matériau — `data/materials/*.json`
...
### B.13 Composant et recette d'obtention — `data/components/*.json`, `data/component_recipes/*.json`
...
- **Mapping Wu Xing (5.2) :** Feu→Feu · Eau/Glace→Eau · Terre→Terre · Métal→Métal · **Foudre et Vie→Bois** (le tonnerre du printemps et la croissance, attribution traditionnelle du Wu Xing) · Arcane/Espace/Corruption→hors cycle (neutres)
...
### C.7 Biomes de départ (12, extensibles vers 20+)

Plaine tempérée, Forêt tempérée, Forêt de mana*, Désert aride, Désert de cendres* (volcanique), Toundra, Taïga, Marécage, Marécage corrompu*, Montagne, Montagne cristalline* (mana+ressources), Côte/plage. (* = biomes "spéciaux" à conditions multi-couches et priorité haute.)
...
**Ajout craft compositionnel (4.2.1) :** **Enclume** (façonnage : lingot → composants métalliques), séparée de la Forge (fonte : minerai → lingot). Les composants en bois viennent de la Scierie, les sangles de l'Atelier de tissage, les composants en pierre du Tailleur de pierre.

**Stations du palier industriel (4.2.2) :** améliorations des stations existantes — Forge → **Haut fourneau**, Enclume → **Laminoir** — débloquées par recettes trouvées/achetées, consommant du combustible (coke, charbon) à chaque opération.
...
- **Voxels : chunks cubiques de 16×16×16 blocs, indexés en 3D `(x, y, z)` dès le premier jour** (jamais en `(x, z)` + pile fixe — coût quasi nul maintenant, retrofit pénible plus tard). La cellule-monde de 128×128 = 8×8 chunks horizontaux × 32 en hauteur par défaut. Le streaming charge une bulle de chunks autour du joueur ; la hauteur 512 est une borne de design que le streaming peut ignorer si un mode profondeur infinie est activé (voir 3.2). Chaque bloc est soit plein (1 id matériau, 2 octets), soit un pointeur vers un octree de subdivision (structure séparée, seule une minorité de blocs sont subdivisés — **budget de 512 blocs subdivisés par chunk**, voir G.2). Meshing par greedy meshing, uniquement les faces visibles, avec **LOD de distance** : la subdivision fine n'est jamais meshée au loin (G.2).
...
### F.1 Matériaux (153 — monde réel uniquement)
...
*Format : nom — type (effet/modificateur/déclencheur) — coût mana base — effet. Dés : notation E.3. Élément Wu Xing (5.2) : hérité du domaine (mapping C.6) — Feu→Feu, Eau/Glace→Eau, Terre→Terre, Foudre→Bois, Vie→Bois, Arcane/Espace/Corruption→neutre. Un domaine Métal (5 modules) est à ajouter au catalogue.*
...
### F.11 Composants standard (14 — craft compositionnel, 4.2.1/B.13)

*Peu de types, beaucoup de matériaux : chaque composant accepte plusieurs familles de matériaux via ses recettes (B.13).*
...
**Note d'équilibrage :** chaque élément porte les 13 stats standard (B.12). Les stats des éléments doivent être cohérentes avec les matériaux qui les contiennent — vérification recommandée : recomposer un matériau connu depuis ses éléments (A.12) doit redonner approximativement ses stats du catalogue F.1 (tolérance ±15 %). C'est le test d'intégrité du système.
