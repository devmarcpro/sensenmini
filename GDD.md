...
- **Transformations principales (station → recettes) :** Forge : minerai→lingot (fonte), sable→verre, argile→brique · **Enclume : lingot→composants métalliques (façonnage, 4.2.1)** · Scierie : tronc→planches, bois→papier, planches→composants bois (manches, hampes) · Tailleur de pierre : roche brute→pierre taillée/pavés/composants pierre · Atelier de tissage : fibres→tissu, paille→chaume, tissu→sangles/rembourrages · Alambic : liquides→extraits/potions · Cuisine : ingrédients→plats (F.5) · Table d'enchantement : gemmes→gemmes taillées (+ enchantement futur, A.4.4). Le détail vit en données (`data/recipes/`), extensible sans code. **Les objets finaux s'assemblent depuis des composants (4.2.1)** — les recettes plates ne subsistent que pour les transformations de matériaux et les consommables.
...
### 4.2.1 Craft compositionnel (amendement du 2026-08-09 — remplace les recettes plates)
...
- Les composants sont **réutilisés partout** : le même « manche court » sert à la pioche, la hache et le marteau — peu de *types* de composants, beaucoup de *matériaux* possibles. Catalogue de composants en F.11.

**Composant logique :** la recette d'un objet référence des **slots typés** (« une tête de pioche »), jamais un matériau (« une tête en fer »). Chaque composant a ses **recettes d'obtention** par familles de matériaux, chacune avec sa table requise et sa condition de déblocage (schéma B.13).

**L'équilibrage par la connaissance :**
- Les **recettes de base** (manche en bois, tête en lingot métallique...) sont connues d'office.
- Les **recettes exotiques** (manche en os, en or, lame d'obsidienne, de verre...) s'**apprennent** : loot de donjon (parchemins de recette, même logique que les grimoires 5.1), achat chez des marchands spécialisés, enseignement de guilde (secrets d'artisans par rang, 7.3) — et certaines demandent une **table plus avancée**. Nouvelle boucle de collection, parallèle aux modules.

**Navigation des recettes (l'UI qui enseigne le système) :**
- La recette affiche ses slots de composants ; **cliquer sur un composant déplie son obtention** (tête de pioche → un lingot, à l'enclume), **récursivement** (lingot → minerai + four ; minerai → où ça se mine). La recette EST le tutoriel — cohérent avec E.19 (information pure, jamais de verrou).
...
**Friction early game — résolue par la boucle existante, pas par un mode simplifié :** le loot de donjon (3.5) et les boutiques couvrent le besoin d'équipement pendant que le joueur apprend le craft. Le craft n'est pas la porte d'entrée obligée de l'équipement.

**Fonte et façonnage séparés (chaîne du métal) :**
- **Four/Forge** : fonte — minerai → lingot (+ sable → verre, argile → brique).
- **Enclume** (nouvelle station, C.8) : façonnage — lingot → composants métalliques (têtes, lames, plaques, rivets).
...
**Qualité — par composant, avec jet d'assemblage (A.4.7) :** chaque composant a sa propre qualité (A.3, sur la compétence de sa station) ; l'assemblage final fait une **moyenne pondérée des qualités des composants + un jet sur la compétence d'assemblage** — un maître assembleur tire le meilleur de composants moyens, un débutant gâche des composants excellents.

**Wu Xing composite (5.2) :** l'alignement élémentaire de l'objet dérive de ses composants — une arme manche-bois/tête-métal est alignée **Bois ET Métal** (chaque élément au prorata du poids du composant). Tout-métal = fort contre Bois mais vulnérable au Feu ; mixte = polyvalent sans bonus franc. Le choix des matériaux de composants devient un choix d'alignement.

**Généralisation :** ce paradigme couvre armes, outils, armures — et les véhicules (E.24) fonctionnaient *déjà* ainsi (coque + roues + mât) : le craft entier du jeu devient un seul modèle compositionnel.

**Niveaux de recette (amendement du 2026-08-09 — les doublons approfondissent) :** chaque recette apprise a un **niveau, de 1 (connue) à 5 (maîtrisée)**, stocké dans le profil du joueur (E.10). Apprendre une recette **déjà connue** fait monter son niveau au lieu d'être jetée — aucun parchemin n'est un loot mort. **Coût croissant** : monter au niveau N+1 demande N doublons (1 → 2 : 1 doublon ; 4 → 5 : 4), soit **10 doublons pour maîtriser** — la montée devient un projet, pas un hasard. Sources de doublons = sources de recettes : loot de donjon, achat, **enseignement d'un artisan à 75+ de relation** (7.2 amendé — le moyen *volontaire* de monter une recette précise, par opposition au hasard du loot). **L'axe unique du bonus (efficacité matière / vitesse et lots / stabilité) se tranche au playtest** — le système est implémenté sans lui : apprentissage, comptage, stockage, affichage ; un point d'accroche unique dans le résolveur de craft attend la décision. **Contrainte non négociable :** le niveau de recette ne multiplie **jamais la qualité** (elle vient de la compétence A.3 et de l'assemblage A.4.7 — deux axes identiques permettraient de court-circuiter la progression de compétence en farmant des parchemins).
...
- **Lisibilité obligatoire** : le tooltip temps réel optionnel (5.0, point 2 —
  ...
  éléments des modules équipés. **Implémenté le 2026-08-10** : le PENTAGRAMME
  est dessiné depuis la table des cycles elle-même (les deux cycles en
  flèches, jamais recopiés) — grand format dans l'onglet combat (vecteur
  composite de l'arme + légende des multiplicateurs), compact sur le HUD en
  bas à droite (chaîne d'engendrement en cours : le dernier élément joué et
  celui qu'il NOURRIT — le prochain coup qui paie — mis en évidence ; masqué
  quand l'arme est neutre et qu'aucune chaîne ne court). *Reste ouvert : le
  tooltip d'alignement de la cible visée.* Les infobulles d'objets (E.13)
  affichent l'alignement composite complet de chaque objet.
...
**Date de naissance et signe (amendement astrologie du 2026-08-09, dépend du
Wu Xing 5.2) :** à la création, le joueur **choisit librement sa date de
naissance** in-game (année + jour), effets affichés en clair. Le choix
détermine son **élément de naissance** (cycle de 5 années : Bois, Feu, Terre,
Métal, Eau) et son **animal** (cycle de 12 années : Rat, Bœuf, Tigre, Lapin,
Dragon, Serpent, Cheval, Chèvre, Singe, Coq, Chien, Cochon) — soit le cycle
sexagésimal complet, 60 combinaisons (« Serpent de Métal », « Dragon de
Feu »...). Le signe complet est affiché sur la fiche du personnage et de tout
PNJ inspecté. L'âge de départ en découle (différence avec la date courante),
purement cosmétique. **Philosophie : la naissance donne une pente, jamais un
plafond** — tout passe par le POTENTIEL (6.4), jamais un bonus dur :
- **Élément de naissance : +10 de potentiel de base** dans ses domaines liés —
  Bois : domaines Foudre et Vie, Agriculture, Herboristerie · Feu : domaine
  Feu, Cuisine, Forge (la flamme) · Terre : domaine Terre, Minage, Maçonnerie ·
  Métal : domaine Métal, Forge (le façonnage), Chimie · Eau : domaine
  Eau/Glace, Alchimie, Navigation, Pêche. Pas de réduction de coût de mana (le
  potentiel suffit ; à réévaluer en playtest).
- **Animal, volet 1 : micro-talents** (+10 de potentiel de base, 2 compétences
  par signe — table en `data/zodiac/animals.json`) : Rat (Discrétion,
  Négociation) · Bœuf (Encaissement, Agriculture) · Tigre (Deux Mains,
  Athlétisme) · Lapin (Esquive, Herboristerie) · Dragon (Contrôle du Mana,
  Leadership) · Serpent (Alchimie, Perception*) · Cheval (Athlétisme,
  Dressage) · Chèvre (Tissage, Tailleur de pierre) · Singe (Lecture,
  Crochetage) · Coq (Armes à distance, Perception*) · Chien (Dressage,
  Encaissement) · Cochon (Cuisine, Négociation). *Perception étant une stat,
  le bonus va au potentiel de stat (A.1.1). Les cumuls élément+animal sont
  permis : un Serpent d'Eau est un alchimiste-né deux fois — rare (1/60) et
  c'est très bien ainsi.
- **Animal, volet 2 : compatibilités relationnelles (7.2)** — trines
  harmonieuses {Rat, Dragon, Singe} · {Bœuf, Serpent, Coq} · {Tigre, Cheval,
  Chien} · {Lapin, Chèvre, Cochon} ; oppositions (6 ans d'écart) Rat–Cheval ·
  Bœuf–Chèvre · Tigre–Singe · Lapin–Coq · Dragon–Chien · Serpent–Cochon.
  Effet : multiplicateur sur la **vitesse de gain de relation** — même trine
  ×1,25, opposition ×0,8, autres ×1,0. Jamais de seuil ni de blocage. Vaut
  AUSSI entre PNJ (couples et affinités pondérés — texture sociale émergente,
  zéro système nouveau) et pour le recrutement de compagnons.
- Le **bànmìng nián** (année malchanceuse de son propre signe) est **exclu**
  (décision actée).
- **12.2** : chaque PNJ ayant un âge, sa date de naissance existe — élément et
  animal en **dérivent** (année mod 5 / mod 12), zéro donnée nouvelle. Signe
  visible à l'inspection, mentionnable en dialogue d'ambiance (E.23).
- Données : `data/zodiac/elements.json` (5) et `animals.json` (12) ;
  localisation composable « {animal} de {élément} » (17 clés suffisent).
- *Questions ouvertes : affichage public du signe des PNJ importants (rois,
  maîtres de guilde) ou découverte par la relation ; équilibrage des +10.*

### 6.2 Emplacements d'équipement — AMENDÉ LE 2026-08-09 (refonte des équipements)

**Principe directeur de l'amendement :** le combat directionnel (E.3.1) vise des **zones**, le craft est **compositionnel** (4.2.1), les matériaux sont **réels** — l'équipement s'aligne sur ces trois faits. **Chaque pièce d'armure protège SA zone de coup** ; la « classe » d'armure n'existe pas comme étiquette (elle **émerge** de la construction et du matériau — un pantalon de plaques d'or est lourd parce que l'or est dense, point) ; **aucun slot placebo**.

**La grille (humanoïde) :**
- **Armure (5), mappée sur les zones de coup E.3.1 :** Casque → tête (la zone ×2.5) · Cuirasse → torse · Brassards-gants → bras et mains · Jambières → jambes · Bottes → pieds (+ effets de déplacement, futurs).
- **Mains (2) :** main principale et main secondaire — chacune accepte arme, bouclier, torche, outil. Deux mains : l'arme occupe les deux slots ; bouclier : main secondaire. *(Dette structurelle notée par l'amendement : le combat doit lire LES SLOTS, plus jamais « l'objet en main » — à payer avant le multijoueur. Les ids persistés restent `arme_1`/`arme_2`.)*
- **Bijoux (3) :** 2 anneaux (effets **mineurs**, A.4.4) · 1 amulette (effet **majeur** unique — le slot du build).
- **Utilitaires (3) :** dos (**cape** = stats/isolation OU **sac** = +capacité de poids — un choix, jamais les deux) · 2 accessoires (objets *portés* : lanterne de ceinture, boussole, trousse, trophée).
- **Munitions :** carquois — *question ouverte de l'amendement (slot dédié ou inventaire) ; les munitions restent en inventaire pour l'instant.*

**Emplacements d'armes :** 2 armes à une main → **Dual Wielding** · arme + bouclier → **Bouclier** · arme à deux mains → **Deux Mains** (en plus de la compétence du type d'arme).

**Équipement non-humanoïde :** morphologies différées par l'amendement (« sans intérêt avant les compagnons E.17 ») — *quadrupède* = tête, torse, selle (dos), amulette, 2 accessoires · *volant* = tête, torse, amulette, 2 accessoires · *amorphe* = amulette, 2 accessoires (`equip_slots` de B.5).

**Constructions d'armure (le « type » émerge du composant) :** une pièce = plaque/corps + sangles + fixations (4.2.1). Le composant principal existe en plusieurs **constructions** — des recettes de composant distinctes (B.13), chacune avec son profil contre les types de dégâts (multiplicateur sur `armure_zone` : fort ×1.3, faible ×0.7 — la matrice A.4.2 devient centrale) :

| Construction | Matériaux typiques | Fort contre | Faible contre | Notes |
|---|---|---|---|---|
| Matelassé | tissus, fibres, laine | contondant | perforant | léger, isolant — l'armure du pauvre et du mage |
| Cuir | peaux | tranchant (léger) | perforant | le milieu polyvalent |
| Mailles | anneaux de métal (recette d'Enclume) | tranchant | contondant | souple, bruyante |
| Écailles | métal, os | tranchant, perforant | contondant | l'exotique intermédiaire |
| Plaque | lingots, os massif | tranchant, perforant | — | lourde, conductrice (Wu Xing Métal, foudre) |

Le matériau reste **libre dans la construction** (mailles de cuivre, plaque d'os) : la construction donne le profil, le matériau donne les chiffres. Constructions exotiques = recettes à débloquer (B.13).

**Décisions :**
- **Accessoires (2 slots) :** objets à effets (A.4.4) qui ne sont ni anneaux ni amulettes ; pools de loot en F.7.
- **Effets passifs (A.4.4) :** portés par anneaux/amulette/accessoires/dos, tirés des pools F.7 **à la génération (loot)** — jamais sur les pièces d'armure de base craftées (l'armure protège, les bijoux augmentent). Implémentation : source `equipement:<uid>` dans le résolveur E.4, posée à l'équipement, retirée au déséquipement.
- **Armure : oui**, craft compositionnel (4.2.1) ET table de sculpture optionnelle (13).
- *Questions ouvertes de l'amendement : carquois dédié ou inventaire ; malus de Discrétion des mailles/plaques ; constructions supplémentaires (lamellaire, brigandine).*
...
**L'information comme récompense (amendement du 2026-08-09, intégré le 2026-08-10)** — l'échelle −100..+100 et ses paliers de prix/hostilité restent inchangés ; ce que la relation débloque en plus, c'est de l'INFORMATION — sur le monde ET sur le PNJ lui-même. C'est la récompense principale, avant les prix.

- **La fiche PNJ se dévoile par paliers** : un PNJ n'est pas une fiche ouverte.
  | Relation | Ce que le joueur voit |
  |---|---|
  | < 0 | Apparence et race, rien d'autre (« un villageois ») |
  | 0..19 | Nom complet, métier, origine |
  | 20..49 | Âge et **signe** (6.1/12.2), niveau approximatif (« paraît aguerri ») |
  | 50..74 | Compétences principales chiffrées, équipement porté (liens familiaux quand 12.3 existera) |
  | 75..89 | Préférences de cadeau explicites, potentiels/spécialités |
  | 90..100 | Tout : historique, relations avec les autres PNJ, possessions |

  Les tranches négatives ne « cachent » pas par punition : un PNJ hostile ne se confie simplement pas.
- **Information sur le monde, par palier ET par métier** (le filtrage par métier d'E.23, réutilisé tel quel — sinon tous les PNJ sont le même distributeur) : 0-19 banalités · 20-49 **informations locales actionnables** (garde → les lois du royaume et leurs sévérités ; marchand → qui vend quoi) · 50-74 **informations à valeur marchande** (mineur/chasseur/aventurier → position d'un donjon ou d'un filon non découvert) · 75-89 **savoirs transmissibles** — les artisans **enseignent une recette de composant exotique** (4.2.1/B.13, troisième source avec le loot et l'achat, la plus « sociale ») · 90-100 faveurs personnelles. Les informations débloquées sont persistées et partageables en coop (E.9).
- **Voie de rédemption** (la partie négative doit avoir une sortie) : **dérive temporelle** — toute relation négative remonte de **+1 par semaine in-game** vers 0, jamais au-delà (le temps apaise, lentement) · **réparation légale** — payer une amende (E.26) répare une part du malus de réputation de l'infraction · **quêtes de rachat** (gabarit B.7 dédié, sous −50 : la réussite remet la relation collective à −20 d'un coup) — *les quêtes de rachat restent à câbler au système de quêtes.*
- **Garde-fous** : la relation collective ne descend jamais sous −100 (borne d'échelle), et **au moins un canal de dialogue reste ouvert** par royaume (un marginal, un ermite — les PNJ de terre sauvage ne portent pas de relation individuelle et ne deviennent hostiles que par les échelons collectifs, plus lents).
- *Questions ouvertes : exclusions de faveurs à 90+ (un roi ne se recrute pas) ; informations revendables (économie de l'information) ?*
...
- **2026-08-09 / intégré le 2026-08-10 — craft compositionnel (4.2.1)** : les recettes plates d'objets finaux sont remplacées par l'assemblage de composants (slots typés, matériau libre, recettes d'obtention par famille — 4.2.1, A.4.7, B.13, C.8 Enclume, F.11). Les recettes plates subsistent pour les transformations de matériaux et les consommables. *Questions ouvertes créées : recettes B.13 par composant × famille et leurs sources exotiques (contenu) ; troisième composant éventuel de la pioche ; compétence d'assemblage — dédiée ou celle de la table d'objet ?*
- **2026-08-09 / intégré le 2026-08-10 — niveaux de recette (4.2.1)** : 5 niveaux par recette apprise, doublons = montée à coût croissant (N doublons pour le niveau N+1, 10 au total), parchemins en double et enseignement d'artisan comptent. L'**axe unique du bonus reste à trancher au playtest** (point d'accroche posé dans le résolveur, aucun bonus actif) ; jamais un multiplicateur de qualité.
- **2026-08-11 — clash lame contre lame (E.3.4)** : deux frappes simultanées s'entrechoquent au lieu d'échanger leurs coups en silence — verdict au momentum mesuré (comparable = double rebond ; différentiel net = le lourd passe en force, ralenti, l'autre est balayé). Recul du joueur encaissant (symétrie E.3.3), autoload de debug visuel `CombatDebug`, sonde `--probe-clash` (couloir 2 voxels + non-tunneling à 20 FPS inclus). Le nombre d'autoloads passe à 32 (le décompte d'ARCHITECTURE, resté à 30, était déjà en retard d'un).
- **2026-08-11 (second lot) — recul du membre touché + rythme des PNJ (E.3.4)** : la zone atteinte cède dans le sens du fer (ressort partagé `limb_flinch.gd`, os parents seuls, borné ~26°, hitboxes suiveuses par cohérence E.3.3) ; et trois corrections de cadence sur retour de l'auteur (« les PNJ spamment trop, changent de parade trop rapidement ») — pause de décision après chaque frappe résolue (fond avec le niveau), plus de déclaration d'attaque pendant une garde tenue, garde 0,9 s / repos 20 ticks. Sonde `--probe-hitbox-os` étendue (flinch : cède / revient / borné).
- **2026-08-11 (troisième lot) — symétrie PNJ/joueur (E.3.5)** : retour de l'auteur, « les PNJ sont très forts, ils n'ont pas les mêmes restrictions que le joueur ». Arme et armure d'un PNJ humanoïde armé deviennent de VRAIES instances forgées (`Equipment`, la même classe que le joueur — `armure_materiaux` ajouté aux 9 fiches concernées), une endurance identique à celle du joueur apparaît côté PNJ (dépensée au swing/mur/clash/garde, gate la déclaration d'attaque, casse la garde), et le stagger joue dans les deux sens (interruption même en pleine frappe). Sonde `--probe-symetrie` (17 vérifications).
- **2026-08-11 (quatrième lot) — restauration de la faune animale (E.3.6)** : les 19 espèces animales retirées le 2026-08-02 « pour l'instant » (faute de rig) reviennent, et 8 de plus avec elles — 27 fiches au total, une par modèle `.glb` non-humanoïde désormais disponible (`models/creatures/`). Profils `bete_sauvage`/`hostile`/`fuit` retrouvent un porteur ; viande et peau reviennent automatiquement (données seules, `tag: bete`). `dragon`/`spectre` restent hors spawn naturel (boss/dungeon). Le catalogue passe à 45 créatures. Sonde `--probe-faune` réécrite.
- **2026-08-11 (cinquième lot) — formes du rig humanoïde reprises de `Humain.bbmodel`** : retour de l'auteur, « j'ai rajouté un modèle Humain Blockbench, j'aimerais qu'on l'utilise pour les humanoïdes ». Le squelette (`BONES`, noms/pivots/hiérarchie — IK, hitboxes par os E.3.3, attache d'arme) ne change JAMAIS ; `tools/generate_humanoid_rig.py` pose désormais les 15 maillages de l'auteur SUR ce squelette existant à la place de la géométrie boîte par défaut, os par os, en conservant les noms de maillage originaux (`mesh_torse_haut`...) dont `player_body.gd` dépend en dur pour le masquage première personne. Rotation/mise à l'échelle calculées (convention de rotation XYZ de three.js/Blockbench, vérifiée par la symétrie gauche/droite du modèle — la seule preuve disponible sans rendu). 3 maillages sans équivalent (`mesh_bassin`, `mesh_cou`, `mesh_cheveux`) restent sur leur boîte d'origine. Hitboxes et `.bbmodel` de secours INCHANGÉS (dérivés de `BODY`, jamais des formes exactes) — seul le rendu change. Vérifié par capture d'écran non-headless en plus des sondes.
- **2026-08-09 / intégré le 2026-08-10 — refonte des équipements (6.2/A.4.2)** : armure = **réduction plate par zone de coup** (les dés globaux, cause mesurée de l'écrasement des dégâts, sont supprimés) ; constructions d'armure (matelassé/cuir/mailles/écailles/plaque) à matériau libre, matrice type de dégâts centrale ; effets passifs A.4.4 branchés sur le résolveur E.4 (source `equipement:<uid>`, pools F.7, bijoux/dos/accessoires en loot) ; magie = armure à 50 %, la défense magique est l'alignement Wu Xing. *Différés par l'amendement lui-même : morphologies non-humanoïdes (attendent E.17), slots de main lus par le combat (avant le multijoueur), carquois (question ouverte).*
...
### A.4.2 Armures et poids porté — AMENDÉ LE 2026-08-09 : réduction plate PAR ZONE

```
À l'impact (E.3.1 détermine la zone touchée) :
  degats_finaux = (degats_bruts * zone_mult - armure_zone) minimum 1
  armure_zone = protection de LA pièce couvrant la zone touchée
              = durete_BASE_composite / 4 * qualite (A.4.7)
              * profil_construction(type_degats)   (fort x1.3, faible x0.7)
  Zone nue (pièce absente) : armure_zone = 0 — la tête sans casque prend
  le x2.5 en plein. S'équiper est un choix de build, pièce par pièce.
  Dégâts MAGIQUES : la réduction de zone s'applique à 50 % — l'armure
  physique aide, jamais la réponse complète ; la vraie défense magique
  est l'alignement Wu Xing de l'armure portée (5.2/A.4.6).
Le jet de dés de mitigation global (1dX par pièce sommés) est SUPPRIMÉ —
  c'était la cause structurelle de l'écrasement des dégâts (2594→192 sur
  400 coups, mesuré) : en directionnel, la défense par compétence (garde,
  distance, zones) s'ajoutait à la mitigation par dés — deux couches là
  où E.3 n'en avait qu'une. La garde (E.3.1) reste LA défense active ;
  l'armure est le filet.
Zones → pièces : tête→casque · torse→cuirasse · bras→brassards-gants ·
  jambes→jambières · pieds→bottes.
...
- La matrice type de dégâts × construction/matériau (`data/armor_type_modifiers.json`) est désormais **centrale** : c'est le profil des constructions (6.2).
- **À distance (physique)** : mêmes règles que la mêlée — la flèche touche la zone qu'elle frappe, type perforant, la matrice s'applique telle quelle.
...
### A.4.7 Craft compositionnel : stats et qualité (4.2.1) — amendement du 2026-08-09
...
STATS DE L'OBJET ASSEMBLÉ :
  ...
  stat_composant = stat du matériau du composant (13 stats, A.4.5) —
    ...
    manche pilote la vitesse : les rôles sont physiquement lisibles.

QUALITÉ :
  qualite_composant : tirée à la fabrication du composant (A.3, sur
    la compétence de sa station : Forge pour une tête, Scierie pour
    un manche...)
...
### B.13 Composant et recette d'obtention — `data/components/*.json`, `data/component_recipes/*.json` (amendement du 2026-08-09)
...
- Un objet (B.3) déclare ses **slots** : `"slots": {"tete": "tete_pioche", "manche": "manche_court", "fixations": "fixations_std"}` avec les poids A.4.7.
- `material_family` : catégorie de matériau acceptée (B.1) — le joueur choisit librement le matériau *dans* la famille ; une famille exotique = une recette distincte à débloquer.
- `unlock_sources` : où la recette se trouve (parchemins en donjon, marchands, rangs de guilde) — même logique d'acquisition que les grimoires (5.1).
- L'UI de navigation (4.2.1) ne fait que parcourir cet arbre : objet → slots → recettes connues (dépliables récursivement) / inconnues (silhouette).
...
**Ajout craft compositionnel (4.2.1, amendement du 2026-08-09) :** **Enclume** (façonnage : lingot → composants métalliques), séparée de la Forge (fonte : minerai → lingot). Les composants en bois viennent de la Scierie, les sangles de l'Atelier de tissage, les composants en pierre du Tailleur de pierre.
...
**Question ouverte créée par cet amendement — le mode tactique (5.0).** Le tour par tour reposait implicitement sur le jet de toucher : une attaque se résolvait en un jet, donc en un tour. Une attaque *visée à la souris* ne se découpe pas ainsi. Trois pistes, aucune tranchée : (a) le mode tactique ne s'applique **qu'à distance et à la magie**, le corps-à-corps restant temps réel ; (b) en tactique, le corps-à-corps retombe sur E.3 d'origine (un jet), en assumant que ce sont deux jeux différents ; (c) le tour fixe la **direction et la garde**, la résolution restant géométrique mais figée. À trancher avant d'écrire la moindre UI d'action time. *(Résolue le 2026-08-09 : le mode tactique est supprimé.)*

### E.3.2 Collision et inertie (amendement du 2026-08-10 — les dés de dégâts quittent la mêlée)

*Décision de l'auteur : « retravailler le système de combat pour que ce soit vraiment par collision et inertie ». E.3.1 avait donné le TOUCHER à la géométrie ; cette révision lui donne aussi les DÉGÂTS. La promesse s'étend : ce que vous voyez toucher touche — et frappe aussi fort que ça allait vite.*

**Le coup est une masse en mouvement.** Le fer est déjà balayé point par point à chaque frame (E.3.1) ; on connaît donc la **vitesse réelle de chaque point de la tête** — locomotion du porteur comprise, puisque la prise suit le corps. À l'impact :

```
COUPS D'INERTIE (tailles, overhead) — énergie cinétique :
  bruts = ½ · masse_arme · v_contact²  · K_taille
COUP DE PRESSION (estoc) — quantité de mouvement concentrée en pointe :
  bruts = (masse_arme + MASSE_CORPS_ENGAGÉ) · v_contact · K_estoc
  (une pointe ne hache pas, elle perce : le corps entier pousse derrière,
  la surface minuscule convertit la poussée — c'est pourquoi un estoc lent
  reste létal là où une taille lente ne fait qu'effleurer)
puis, inchangé :
  bruts *= (durete_BASE/20) · qualité      <- le TRANCHANT convertit l'énergie
  bruts += For/4                            <- le muscle derrière le geste
  bruts *= zone_mult · mult_Wu_Xing        <- anatomie (E.3.1) et élément (A.4.6)
  finaux = max(1, bruts - armure_zone)     <- A.4.2 amendé, inchangé
v_contact = déplacement du POINT DE CONTACT entre deux frames / delta
  (borné par V_CAP contre les téléportations), + vitesse de rapprochement
  du porteur côté créatures (leur corps avance au tick, pas à la frame).
```

**Ce que la mesure rend GRATUIT (les artifices tombent) :**
- **Le sweet spot cesse d'être une formule** : dans un arc, le bout du fer va réellement plus vite que le talon — frapper du talon fait peu parce que le talon va lentement, pas parce qu'une courbe le décrète. `sweet_spot_factor` ne survit que comme règle binaire « le manche ne coupe pas » (contact avant la jonction = coup glissant, 0 dégât).
- **Le bonus de vitesse cesse d'être un facteur** : charger en frappant augmente v_contact directement (la prise suit le corps). Reculer en frappant la diminue. `speed_factor` disparaît de la mêlée.
- **Les dés de dégâts disparaissent de la mêlée** : la variance vient désormais d'OÙ et QUAND on attrape la cible dans l'arc — début d'arc lent, pleine course rapide. Les dés restent aux **modules et sorts** (la magie n'a pas d'inertie) ; le **+1 dé du combo Wu Xing (A.4.6) devient ×1,3** sur les dégâts de mêlée, même intention, même ordre de grandeur.

**L'arc est inertiel.** La lame ne parcourt plus son arc à vitesse constante : la phase de frappe suit un profil **accéléré par la masse** (`u_eff = t^k`, k croissant avec le poids de l'arme). Une dague est à pleine vitesse presque immédiatement ; un marteau de guerre démarre lentement et délivre tout en fin d'arc. Conséquences voulues : interrompre un coup lourd TÔT (chambering, esquive du premier tiers) le vide de son énergie ; le timing d'une arme lourde devient une compétence.

**La collision arrête la lame — avec conséquence.** Jusqu'ici, un mur ou une parade « consommait » le coup mais la lame poursuivait son arc, fantomatique. Désormais :
- **le décor** : la lame qui rencontre un bloc REBONDIT — la frappe passe immédiatement en récupération RALLONGÉE (×1,4), clang, léger coût d'endurance supplémentaire. Se battre dans un tunnel avec une hallebarde devient exactement le problème que Mount & Blade promet ; l'estoc, qui n'a pas d'arc latéral, devient l'arme des couloirs *par la géométrie* ;
- **la parade tenue et le chambré** : même rebond côté attaquant — l'arme arrêtée revient, elle ne traverse plus l'adversaire en silence ;
- **la chair** ne rebondit pas : la lame traverse (hit-stop existant) — un corps n'est pas un mur.

**Projectiles physiques (arcs, arbalètes) — même règle** : les dégâts d'une flèche dérivent de sa **vitesse réelle à l'impact** (elle décélère, tombe — une flèche en cloche en bout de course fait moins mal qu'un tir tendu) × masse de la munition × conversion de la pointe. Les projectiles de MODULES restent aux dés (magie).

**Calibrage** : les constantes K sont réglées pour qu'une épée fer/chêne de qualité 1, frappée en pleine course sur un torse, retombe dans la fourchette de l'ancien modèle (~6-14) — le rééquilibrage est un recentrage, pas une inflation. Vérifié par sonde (`--probe-inertie`), enveloppe assertive.

**Ce qui ne change PAS** : la lecture directionnelle, les gardes et leur endurance, le crushthrough, les zones de coup, l'armure par zone (A.4.2), le Wu Xing en étape 3, la règle « la frame constate, le tick applique ».

### E.3.3 Les membres ont une collision réelle (amendement du 2026-08-10)

*Décision de l'auteur : « que les armes et les membres des PNJ et du joueur aient de réelles collisions — qu'une lame ne puisse pas passer à travers un mur, et qu'une arme fasse des dégâts quand elle touche un membre, avec de l'inertie ». E.3.2 avait rendu les dégâts physiques ; cette révision rend la CIBLE physique.*

**La cible du combat est le corps AFFICHÉ, os par os.** Les zones de coup n'étaient que des boîtes statiques autour de la position logique — jamais tournées par le lacet (un loup de profil gardait sa hitbox en travers de son corps), jamais déplacées par l'animation, et ancrées un demi-bloc sous le corps visible (convention `FEET_OFFSET`). Trois mensonges géométriques superposés : la cause première du « je vise ce que je vois et ça rate ». Désormais **chaque os du squelette porte sa boîte** (`systems/combat/body_volumes.gd`), calculée par les générateurs sur la géométrie de chaque maillage (champ `os` de `rigs.json`, `humanoide.json`) et posée par le squelette à chaque frame :

- le **yaw, la marche, l'ondulation, la pose de combat** déplacent les hitboxes comme ils déplacent les pixels — la patte levée se touche en l'air, la morsure projetée avance sa tête, la queue du crocodile balaie vraiment ;
- **mêmes volumes pour les trois camps** : créatures (`CreatureBody`), PNJ humanoïdes et joueurs distants en duel (`PlayerBody`), et le joueur LOCAL (les créatures frappent le corps que les autres voient de lui) ;
- **un membre démembré (os à l'échelle zéro) n'arrête plus rien** — sa boîte disparaît avec lui ;
- les ids de zone de l'humanoïde sont CEUX DU GABARIT et de l'**armure par zone** (tete/torse/bras_*/jambes/pieds) — l'armure couvre exactement ce que les coups atteignent ;
- **repli** (créature sans corps construit) : les zones statiques du gabarit, désormais TOURNÉES par le lacet affiché ;
- pas de moteur physique : test analytique segment↔boîte orientée, payé à la demande (frappe en cours, cibles à portée), inverse des poses cachées par frame — compatible avec le budget de la machine cible et le tick autoritaire.

**Le décor arrête TOUTE la lame, tout le temps.** Pendant la frappe, chaque point échantillonné du fer teste son propre segment contre les blocs (plus seulement la pointe : le milieu d'une lame qui mordait un angle de mur passait au travers) ; mur touché AVANT la chair **dans la frame** (comparaison en fraction de frame, la même unité que le `t` des zones) = rebond E.3.2. HORS frappe, l'arme tenue se **REPLIE devant un mur** (à la Mount & Blade) : plus proche que la longueur de lame, la main rentre vers le corps et la lame se redresse (`_wall_cramp`, lissé) — le fer ne repose plus jamais DANS un bloc en garde ou en marchant, et une frappe déclenchée sans place rebondit au premier centimètre.

**La vitesse d'impact est RELATIVE, des deux côtés.** La cible qui charge dans le coup s'empale (sa vitesse s'ajoute), celle qui rompt l'amortit — le vieux « speed bonus » de Mount & Blade, mais mesuré, et symétrique : le joueur qui recule devant une morsure la subit plus faible.

**Un coup qui touche marque toujours** : minimum 1 dégât, armure ou pas (extension de la règle A.4.2 à la chair nue — l'effleurement qui s'arrondissait à 0 disait « raté » à un joueur qui venait de toucher). Et le **calibrage est relevé ×2** (`SWING_ENERGY_K` 0.00035 → 0.0007, estoc 0.014 → 0.018) : l'épée de référence à mi-course passe de ~6 à ~11 — un coup net blesse franchement, la variance vient toujours de l'arc.

**Le « coup glissant » a posteriori est retiré.** La règle « le manche ne coupe pas » est garantie par construction (seule la tête vulnérante balaie, `head_span`) ; l'ancien contrôle comparait la distance MONDE prise→contact à `head_start`, qui la sous-estime dès que le geste recule l'arme (tout l'estoc part reculé de `thrust_pull`) — il requalifiait la pointe en manche et mangeait la frappe entière, surtout depuis que les membres posés s'avancent réellement vers la lame.

**L'impact se SENT — dans les mains, à la caméra, sur le monde** (décision de l'auteur : « on doit sentir l'impact et aussi si on se fait parer »). Au hit-stop et à la secousse-bruit existants s'ajoutent quatre retours PHYSIQUES, tous dérivés de la géométrie du choc :

- **le recul du bras** : l'impulsion du choc entre dans le ressort d'inertie de la main — celui-là même qui donne le ballant à l'arme. Un coup qui MORD décélère le bras dans le sens du geste ; un coup ARRÊTÉ (paré, mur, chambré) le repousse à contre-sens, plus fort — puis le ressort ramène la main avec la mollesse propre à l'arme tenue. Purement visuel, jamais réinjecté dans la hitbox ;
- **le coup de caméra ORIENTÉ** (`ImpactFeedback.kick`) : la secousse est un bruit, elle dit la violence sans dire le sens. Le kick est la composante orientée — mordre accompagne le geste (petit), se faire parer REPOUSSE la tête en arrière du geste (fort), encaisser chasse le regard du côté opposé à l'impact, parer se sent dans un petit recul. Retour au neutre en ressort (~150 ms), amplitude bornée (la caméra reste le réticule), même réglage de confort que la secousse ;
- **la lame revient DU POINT DU CHOC** (`MeleeAttack.recovery_from`) : après un rebond, la récupération repart de la progression où l'arc a été arrêté — plus de main téléportée en fin d'un arc jamais parcouru, à travers l'obstacle qui venait de l'arrêter ;
- **le recul de la cible** (`Creature.apply_knockback`) : un coup qui mord POUSSE dans le sens du fer, proportionnellement à l'énergie brute et à l'inertie du gabarit (santé max), borné (~0,45 bloc — un coup repousse, il ne catapulte pas), via `_slide` donc jamais dans un mur. Appliqué AU TICK, avec les dégâts : c'est un état de jeu.

Le SON reste absent, volontairement : le projet n'a pas de système audio, il viendra comme système à part entière (même décision qu'à E.3.1).

> **Implémenté le 2026-08-10** — générateurs (`generate_creature_rigs.py`,
> `generate_humanoid_rig.py` : champ `os` par maillage→os), `body_volumes.gd`,
> branchements dans `creature.gd`/`player_body.gd`/`player.gd`, sonde
> `--probe-hitbox-os` (22 vérifications), `--probe-inertie` recalibrée,
> `--probe-combat` adaptée (montage : pose immédiate du corps, cibles posées à
> la convention FEET_OFFSET — leur corps affiché flottait un demi-bloc trop
> haut, invisible tant que les boîtes étaient ancrées au logique).

### E.3.4 Clash lame contre lame (amendement du 2026-08-11)

*Plan validé par l'auteur (« système de combat physiquement crédible, style Mount & Blade ») : E.3.2 avait rendu les dégâts physiques, E.3.3 la cible physique — cette révision rend l'ARME ADVERSE physique. Deux fers en plein vol qui se croisent ne s'ignorent plus : ils se percutent.*

**Le dernier contact sans collision.** Jusqu'ici, deux frappes simultanées échangeaient leurs coups en silence — chacune balayait les zones de l'autre corps, aucune ne voyait l'autre lame. Le chambré (même direction, fenêtre de wind-up) était le seul entrechoc possible. Désormais, pendant la frappe, le fer teste aussi le **fer adverse en vol** (proximité segment↔segment, rayon charnu — deux instantanés de frame ne se croisent jamais au millimètre), et c'est la **première priorité de collision**, avant le décor et la chair — l'ordre de Mount & Blade.

**Le verdict est le MOMENTUM MESURÉ** (masse d'arme × vitesse de pointe — les mêmes grandeurs que les dégâts E.3.2, mesurées au même endroit) :
- **momentums comparables** (rapport < `CLASH_DOMINANCE`, 1,8) → les **DEUX lames rebondissent** (récupération E.3.2, hit-stop au plafond de la fourchette M&B, clang, surcoût d'endurance comme un mur). Le cas nominal du duel ;
- **différentiel net** → le fer lourd **PASSE EN FORCE** : il balaie l'arme adverse (stagger : récupération rallongée côté balayé) et **poursuit son arc, ralenti** (`CLASH_DRAG` ×1,35 sur le temps restant — la progression ne saute pas en arrière, seul l'élan est mangé). Le marteau ne s'arrête pas sur une dague ; ne PAS engager son fer léger dans un fer lourd lancé devient une lecture à faire.

**Les fenêtres sont exclusives par construction** : le chambré couvre le wind-up adverse (`is_chambering`), le clash couvre sa phase de frappe (`blade_probe`, active seulement tant que le coup n'a pas déjà abouti). Un même échange n'est jamais jugé deux fois. Les joueurs distants en duel restent au chambré seul (l'état fin de leur frappe n'est pas répliqué à la frame).

**Symétrie du recul (complément E.3.3)** : le joueur encaissant recule désormais sous les coups comme la créature (`Player.apply_knockback`, même formule — l'inertie est la santé maximale —, même borne, appliqué au tick, collision de la marche respectée). Jamais sur une parade tenue, qui absorbe l'élan.

**Le membre touché CÈDE (recul procédural, 2026-08-11)** : la zone atteinte bascule dans le sens du fer puis un ressort la ramène (~250 ms, amplitude bornée ~26°) — léger, jamais un ragdoll (`systems/animation/limb_flinch.gd`, partagé par `CreatureBody` et `PlayerBody`). Seuls les os PARENTS de la zone basculent (sous-arbres disjoints — pas de cumul le long d'une chaîne, pas de dilution par le nombre d'os). Contrairement au recul du bras du joueur (E.3.3, hors hitbox : c'est sa visée), ce recul-ci déplace le corps AFFICHÉ, donc les boîtes par os le suivent — cohérent avec « ce qu'on voit est la cible » ; l'amplitude bornée garantit que c'est un ressenti, pas une esquive gratuite. Vaut pour les créatures, les PNJ humanoïdes et l'avatar adverse en duel (localement).

**Rythme des PNJ (2026-08-11, retour de l'auteur : « les PNJ spamment trop et changent de parade beaucoup trop rapidement »)** — trois corrections dans `creature.gd` :
- **pause de décision après chaque frappe RÉSOLUE** (touché, raté, mur) : le cooldown courait depuis la déclaration et arrivait déjà consommé à la récupération — la créature redéclarait aussitôt, un moulin à coups. La pause fond avec le niveau (14 ticks − niveau/3, min 5) : un villageois souffle, un chef enchaîne ;
- **une garde tenue est un engagement** : la créature pouvait déclarer une attaque un tick après avoir levé sa garde, la pose de windup écrasait la pose de garde — un clignotement, pas un choix. Plus de déclaration tant que la garde est levée (règle déjà imposée dans l'autre sens) ;
- **garde tenue plus longtemps** (0,6 → 0,9 s) et repos entre gardes allongé d'autant (14 → 20 ticks depuis la levée) : une parade se LIT, et la fenêtre d'enchaînement (garde baissée) reste ouverte ~1,1 s.

> **Implémenté le 2026-08-11** — `MeleeAttack.segment_proximity`/`clash_drag`,
> `CombatResolver.CLASH_RADIUS`/`CLASH_DOMINANCE`, `Creature.blade_probe`/
> `clash_rebound`, `Player._try_blade_clash`/`apply_knockback`,
> `FlyCamera.nudge`, `ImpactFeedback.IMPACT_CLASH`, autoload `CombatDebug`
> (gizmos de balayage/impacts + journal vitesse→dégâts, menu de triche F1).
> Sonde `--probe-clash` : les trois issues du clash, l'exclusivité des
> fenêtres, le couloir de 2 voxels (la taille rebondit, l'estoc passe) et le
> non-tunneling à 20 FPS. Le SON reste absent, comme à E.3.1/E.3.3.

### E.3.5 Symétrie PNJ/joueur — équipement réel et endurance (amendement du 2026-08-11)

*Décision de l'auteur : « les PNJ sont très forts car ils n'ont pas les mêmes restrictions que le joueur — il faut supprimer le plus possible l'asymétrie, les PNJ doivent être construits comme le joueur, ils ont aussi des équipements ». Audit confirmé : les PNJ humanoïdes armés traversaient une version allégée du pipeline de combat — arme et armure réduites à des chiffres de fiche, aucune endurance, garde gratuite et increvable, stagger à sens unique, mur sans coût.*

**L'équipement est désormais une VRAIE instance forgée**, pas un dictionnaire de fiche. Au spawn, `Creature._build_equipment()` construit un `Equipment` (la MÊME classe que le joueur) depuis les données de combat :
- l'**arme** (`combat.arme_materiaux`) est forgée par `ItemFactory.craft` — masse, dureté de base et coût d'endurance en découlent RÉELLEMENT (avant : `WeaponStats.derive(functionality, {})`, une instance toujours vide, ignorait `arme_materiaux` pour tout ce qui n'était pas le modèle 3D) ;
- l'**armure** (nouveau champ `combat.armure_materiaux`, ajouté aux 9 fiches humanoïdes armées) forge les 5 pièces (casque/cuirasse/gants/jambières/bottes) et les pose dans les slots de zone (A.4.2) ;
- la **qualité** suit le niveau de combat (`clamp(0.5 + niveau/25, 0.5, 1.8)`) — un chef de bande porte un meilleur acier qu'un premier bandit croisé ;
- une créature à ARME NATURELLE (griffes, morsure — pas d'`arme_materiaux`) reste sans `Equipment` (`null`, jamais un dictionnaire vide) : l'absence est honnête, un loup n'a jamais porté de plates qu'on lui retirerait ici.

**Conséquence directe : l'armure d'un PNJ RÉDUIT les dégâts qu'il encaisse.** Le joueur qui frappe un PNJ armé lit désormais `creature.armor_reduction(zone, type)` au lieu d'un `-1.0` en dur qui ne mitigait jamais rien, pour personne.

**L'endurance existe côté PNJ** (`Creature.stamina`/`stamina_max`, mêmes constantes que `Player` — base 50 + endurance×10) et suit exactement les mêmes règles :
- chaque coup PARTI la dépense, touché ou non (`_release_declared_attack`) ;
- **à sec, la créature NE DÉCLARE PAS de coup** (le frein qui manquait totalement — avant, une créature frappait sans fin pendant que le joueur seul s'essoufflait) ;
- sa **garde coûte à tenir** (`absorb_guard_hit`, drain de l'arme adverse) et **CASSE immédiatement** si l'endurance est insuffisante — avant, une garde de PNJ ne coûtait jamais rien et bloquait indéfiniment ;
- un **mur** ou un **clash** lui coûte la moitié d'un swing et rallonge sa récupération (×1,4, `MeleeAttack.REBOUND_RECOVERY_MULT`) — exactement le tarif du joueur, jusqu'ici gratuit pour elle ;
- régénération suspendue garde levée, même délai de blocage après dépense (`_stamina_tick`, copie de `Player._stamina_tick`).

**Le stagger joue désormais dans les deux sens.** Le joueur interrompt SA frappe sur tout dégât encaissé, même lame déjà partie ; la créature ne le faisait qu'en wind-up/menace tenue — un coup qui la touchait EN PLEINE FRAPPE ne lui coûtait jamais son attaque. `take_damage` interrompt désormais aussi la phase `strike`.

> **Implémenté le 2026-08-11** — `Creature.equipment`/`weapon_instance`/
> `armor_reduction`/`_build_equipment`, `Creature.stamina`/`_spend_creature_stamina`/
> `_stamina_tick`/`absorb_guard_hit`, données `armure_materiaux` (bandit,
> braconnier, chasseur, chef_de_bande, deserteur, ermite, forgeron,
> garde_village, pillard). Sonde `--probe-symetrie` (17 vérifications :
> équipement réel, mitigation d'armure, portes d'endurance, casse de garde,
> stagger en pleine frappe, coût du mur). Non-régression : `--probe-combat`,
> `--probe-clash`, `--probe-hitbox-os`, `--probe-inertie`, `--probe-faune`,
> `--probe-equipement` verts.
>
> **Non fait, hors scope de cet amendement** : les créatures non-humanoïdes
> (bêtes) restent à arme naturelle sans endurance — leur symétrie n'a pas de
> sens tant qu'elles n'ont ni garde ni équipement dans le GDD ; l'IA ne réagit
> pas encore stratégiquement à sa propre endurance basse (elle continue de
> chercher à attaquer, elle échoue juste à déclarer) — un raffinement de
> comportement, pas une brèche de symétrie.

**Complément du même jour — l'équipement dépend de la CLASSE et du NIVEAU, pas de la fiche** (retour de l'auteur : « un humanoïde aura un équipement qui dépend de sa classe et son niveau » ; puis « un paysan n'aura pas le même équipement qu'un guerrier ou un mage ou un garde ou un bandit mais ne sera pas pour autant tout nu »). Première version : `arme_materiaux`/`armure_materiaux` étaient des dictionnaires figés, un par fiche — retapés à la main pour chaque nouvelle recrue, et un PNJ à mains nues (villageois, fermier...) n'avait aucun équipement du tout.

Remplacé par une dérivation en DEUX AXES, dans `Creature._build_equipment` :
- la **CLASSE** (`data/classes/*.json`, les 6 classes déjà utilisées par la création de personnage, C.3) choisit le **GABARIT** : quelles zones sont couvertes et si l'armure porte du métal (`CLASS_ARMOR_PROFILE`) — guerrier en couverture complète et métal, chasseur en cuir léger sans casque, mage en robe minimale, artisan en tenue de travail, marchand et vagabond en simple tunique (le paysan n'est jamais nu, juste moins couvert) ;
- le **NIVEAU DE COMBAT** choisit le **PALIER de matériau** (`_gear_tier` : <8 faible/cuivre, 8–16 moyen/fer, ≥16 fort/acier) — un chef de bande (16) forge un meilleur acier qu'un premier bandit (10).

**Toute créature humanoïde (`classe` en données) reçoit un équipement**, arme SEULEMENT si elle a une fonctionnalité de combat assemblable (mains nues n'en a pas) — l'armure, elle, est toujours forgée. Le TYPE d'arme reste celui de la fiche (`combat.functionality` — un garde porte une lance, un chef de bande une hache d'armes : c'est son identité), seul le MATÉRIAU vient du palier. Le modèle 3D en main (`_attach_weapon`) partage exactement la même dérivation (`Creature.gear_materials`) que le mécanique : ce qu'on voit et ce qui frappe ne peuvent plus diverger, même règle qu'ailleurs dans ce fichier pour `functionality`.

> **Implémenté le 2026-08-11** — `Creature.CLASS_ARMOR_PROFILE`/
> `GEAR_TIER_WOOD`/`GEAR_TIER_METAL`/`_gear_tier`/`gear_materials`/
> `_build_equipment(data)` (signature changée). Champs `arme_materiaux`/
> `armure_materiaux` retirés des 9 fiches qui les portaient — plus lus par
> aucun code. Sonde `--probe-symetrie` étendue (gabarit par classe, palier par
> niveau, villageois à mains nues désormais équipé). Non-régression :
> `--probe-clash`, `--probe-hitbox-os`, `--probe-inertie`, `--probe-faune`,
> `--probe-equipement`, `--probe-butin` verts. `--probe-combat` échoue de
> façon PRÉ-EXISTANTE (nœud « previously freed » dans `_check_ranged`, déjà
> observé de façon intermittente avant cet amendement) — le coût de spawn
> accru par le nouvel équipement (5-6 forges par PNJ humanoïde) semble l'avoir
> rendu systématique ; cause profonde non identifiée, hors scope de ce
> commit, à reprendre séparément.

**Second complément du même jour — les CONSTRUCTIONS nommées de 6.2, pas leur repli.** En relisant §6.2 avec l'auteur, deux écarts trouvés entre l'implémentation ci-dessus et l'amendement d'origine :

1. Les 5 pièces d'armure (`data/items/armure/*.json`) verrouillent leur slot « plaque » sur la catégorie `minerai` — la voie de craft « un seul bloc » (`ItemFactory.craft`, utilisée ci-dessus) ne peut donc JAMAIS y poser du tissu. Les classes légères (chasseur, mage, marchand, vagabond) retombaient sur le repli catégorie-dominante (« textile », profil 0.7/0.5/1.4) au lieu de la construction **matelassée** nommée que la table de 6.2 prévoit (1.0/0.7/1.3) — un repli documenté et fonctionnel, mais pas ce que l'amendement décrit.
2. La construction « plaque » (voie compositionnelle) exige la famille `lingot`, et `lingot_acier` n'existait pas — seuls `lingot_fer`/`lingot_cuivre` étaient raffinés. Le palier « fort » de la CLASSE GUERRIÈRE n'avait donc pas d'acier compositionnel disponible.

**Corrigé** : `_build_equipment` assemble désormais l'armure par **composants** (`ItemFactory.craft_component` + `assemble`, la voie « atelier » de 4.2.1/A.4.7) au lieu de la voie d'un bloc — cette voie ignore la contrainte de catégorie du slot et pose la construction EXPLICITE portée par la recette du composant (`plaque_lingot` → « plaque », `plaque_textile` → « matelasse »). Nouveau matériau `lingot_acier` (`data/materials/overworld/lingot/`, stats dérivées de `acier` par le même rapport que `lingot_fer`/`fer` : dureté ×1,3, valeur ×2, le reste inchangé) complète le trio de paliers en lingot.

> **Implémenté le 2026-08-11** — `data/materials/.../lingot_acier.json` +
> locale 4 langues ; `Creature.GEAR_TIER_INGOT` ; `_build_equipment` réécrit
> pour l'armure (composants `plaque`/`sangles`/`fixations`, recettes
> `plaque_lingot`/`plaque_textile`/`sangles_textile`/`fixations_lingot`/
> `fixations_textile`, toutes `unlocked_by_default`). Sonde `--probe-symetrie`
> étendue : la construction du guerrier vaut exactement « plaque », celle du
> vagabond exactement « matelasse » — la table nommée de 6.2, pas son repli.
> Non-régression : `--probe-clash`, `--probe-hitbox-os`, `--probe-inertie`,
> `--probe-faune`, `--probe-equipement`, `--probe-butin`, `--probe-assemblage`,
> `--probe-composants` verts.

### E.3.6 Restauration de la faune animale (amendement du 2026-08-11)

*Demande de l'auteur : « tu peux en profiter pour rajouter les différentes espèces qui ont des modèles ». Le périmètre humain du 2026-08-02 (`Perimetre reduit aux humains : 19 especes animales supprimees`) était explicitement provisoire — sa propre note l'annonçait : « POUR L'INSTANT... le seul rig disponible est humanoide.glb ». 27 modèles d'animaux existent désormais dans `models/creatures/` (chacun avec son manifeste de hitboxes par os dans `rigs.json`, E.3.3) : le blocage cité par la suppression n'existe plus.*

**27 nouvelles fiches `data/creatures/*.json`**, une par modèle non-humanoïde disponible : abeille, aigle, araignée, autruche, cerf, chat sauvage, chauve-souris, cheval, crabe, crocodile, dragon, lézard, loup, méduse, mille-pattes, ours, pieuvre, poisson, raie, raptor, rat, requin, sanglier, scarabée, serpent, spectre, ver. Chacune porte une arme naturelle (`morsure` — la seule fonctionnalité de ce type au catalogue, comme les fiches d'origine), des `base_stats` et un `niveau_combat` gradués par gabarit et menace, et des `world_gen.biome_tags` cohérents avec son habitat. Locale : `creature.<id>.name` dans les 4 langues.

**Profils d'IA retrouvés en contenu, pas seulement en code** : `bete_sauvage` (loup, ours, araignée...) neutre puis hostile si dérangée ; `hostile` pur pour les prédateurs et monstres (crocodile, requin, raptor, dragon, spectre) ; `fuit` (cerf, poisson, cheval, autruche, lézard, pieuvre, raie) — herbivores et proies qui s'écartent et ne ripostent jamais, même provoqués. Le mécanisme existait déjà depuis la suppression de 2026-08-02 ; il n'avait simplement plus de porteur.

**Deux espèces restent hors du spawn naturel par construction** : `dragon` et `spectre` portent des `biome_tags` vides (boss/dungeon uniquement — même statut que `chef_de_bande`, matière à donjon plutôt qu'à rencontre de plein air).

**Effet de bord voulu, pas un correctif** : la viande et la peau (`GameData._generate_parametric_resources`) reviennent automatiquement pour toute bête à corps — c'est déjà entièrement piloté par les données (tag `bete`), aucune ligne de code n'a dû changer. Seul `spectre` (fantôme, tag `amorphe`) échappe au dépeçage — la règle existait, elle n'avait plus de porteur non plus.

> **Implémenté le 2026-08-11** — 27 fiches `data/creatures/*.json`, entrées
> `creature.<id>.name` dans `locale/{fr,en,ja,zh_Hans}.csv`, commentaire à jour
> dans `CreatureManager._ready` (l'exclusion manuelle de spawn redevient
> vide). Sonde `--probe-faune` réécrite pour la faune restaurée (catalogue,
> pool mixte humains/bêtes, cohérence par biome, profils `fuit`/`bete_sauvage`
> avec provocation). Non-régression : `--probe-butin` (règle « un amorphe ne
> se dépèce pas » enfin couverte, sur `spectre`), `--probe-equipement`,
> `--probe-symetrie`, `--probe-combat`, `--probe-clash`, `--probe-hitbox-os`,
> `--probe-inertie` verts.
>
> **Défaut pré-existant découvert en validant, hors scope de cet
> amendement** : `--probe-rigs-animaux` signale l'araignée comme non animée
> (« FIGEE ») — un défaut du rig `araignee.glb`/de son mapping de pattes
> (8 pattes, comme le crabe, qui lui anime correctement), pas des données
> ajoutées ici. À reprendre séparément.
...
**Amendé le 2026-08-10 (demandes de l'auteur) :**
- **Infobulles complètes au survol de tout objet** (inventaire, hotbar,
  équipement, coffres) : UNE seule fonction de description (les chiffres que
  le combat utilise réellement — dés/vitesse/allonge dérivés, réduction
  d'armure par type, construction, composants montés avec leur qualité,
  alignement Wu Xing composite, effets A.4.4, gemme, sort inné, nutrition et
  potentiel, poids/valeur) ; matériaux : catégorie, élément, les 13 stats B.1.
- **Le personnage EN PIED est visible** dans l'onglet personnage, dans
  l'inventaire (au-dessus de la silhouette d'équipement) et à la création —
  rendu partagé unique (même politique que le portrait de dialogue).
- **Création de personnage en deux colonnes** (identité/naissance à gauche,
  points/attributs à droite) : tout tient à l'écran.
...
### F.11 Composants standard (14 — craft compositionnel, 4.2.1/B.13, amendement du 2026-08-09)

*Peu de types, beaucoup de matériaux : chaque composant accepte plusieurs familles de matériaux via ses recettes (B.13). (F.10 est réservé aux éléments chimiques de l'amendement chimie, non encore intégré.)*

**Manches (3) :** Manche court (pioche, hache, marteau, masse, dague) · Manche long (lance, hallebarde, faux) · Poignée (épées, couteaux)
**Têtes et lames (5) :** Tête d'outil (pioche/hache/marteau — la forme finale vient de la fonctionnalité choisie) · Lame courte (dague, couteau) · Lame longue (épées) · Tête d'arme lourde (masse, marteau de guerre) · Pointe (lance, flèches, trident)
