...
const MUT=[
  ...
  ['failles : le monde en rouvre','failles',[['src/27-clock.js',"      z.poi='donjon';z.dj=null;z.djDone=null;z.cleared=0;","      z.cleared=0;"]]],
  ['failles : ce qu on tient ne s ouvre pas','failles',[['src/27-clock.js','    if(z.poi||z.claim||z.town||presse[k])continue;','    if(z.poi)continue;']]],
  ['failles : jamais plus de quatre a la fois','failles',[['src/27-clock.js','  if(ouvrables.length&&failles<4){','  if(ouvrables.length){']]],
  ['failles : la chance se lit sur l ecart au bruit','failles',[['src/27-clock.js','    if(Math.random()<Math.min(.35,(delta(z)-SEUIL)/PENTE)){','    if(Math.random()<.35){']]],
